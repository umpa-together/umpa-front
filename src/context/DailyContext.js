...
const dailyReducer = (state, action) => {
    switch(action.type) {
        ...
        case 'getAllDailys':
            return { ...state, allDailys: action.payload, notAllDailysNext: false, currentAllDailysPage: 1 }
        case 'nextAllDailys':
            return { ...state, allDailys: state.allDailys.concat(action.payload), currentAllDailysPage: state.currentAllDailysPage + 1 }
...
const nextAllDailys = (dispatch) => async ({page}) => {
    try {
...
const editDaily = dispatch => async({  textcontent, songs, hashtag, DailyId }, callback) => {
    try {
...
const deleteDaily= dispatch => {
    return async ({ id }) => {
        await serverApi.delete('/Daily/'+id);
    };
};

const likesDaily = dispatch => {
    return async ({ id }) => {
        try{
...
const nextDailys = (dispatch) => async ({ page }) => {
    try {
...
const addComment = dispatch => {
    return async ({ id, text, noticieduser, noticieduseremail, noticetype, thirdid }) => {
        try{
            const response = await serverApi.post('/Dailycomment/'+id, { text, noticieduser, noticieduseremail, noticetype, thirdid });
