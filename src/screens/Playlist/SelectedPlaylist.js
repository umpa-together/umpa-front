...
const SelectedPlaylist = ({ route }) => {
  ...
  const fetchData = () => {
    getPlaylist({
      id: state.current_playlist._id,
      postUserId: state.current_playlist.postUserId._id,
    });
  };
  ...
  */
  ...
  useEffect(() => {
    if (searchState.hashtag != null && hashtag !== '') {
      navigate('SelectedHashtag', { data: searchState.hashtag });
    }
  }, [searchState.hashtag]);

  useEffect(() => {
    if (state.current_playlist != null && state.current_playlist.length === 0) {
    ...
    <View style={styles.container}>
      {currentPlaylist == null ? (
        ...
        <View style={styles.mainContainer}>
          ...
          {isPlayingId === '0' ? <CommentBar playlistId={playlistid} /> : <MusicBar />}
        </View>
      )}
    </View>
  );
