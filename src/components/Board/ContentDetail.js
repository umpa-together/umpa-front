...
const ContentDetail = ({ title }) => {
  const { state, likeContent, unlikeContent, createComment, createReComment, getCurrentContent } =
  ...
  useEffect(() => {
    if (
      ...
      state.currentContent.image.length != 0
    ) {
      for (const key in state.currentContent.image) {
        const tmpImage = { url: state.currentContent.image[key] };
        img.push(tmpImage);
      }
      setImage(img);
    }
  ...
  return (
    <View style={{ backgroundColor: 'rgb(255,255,255)' }}>
      {state.currentContent == null || state.currentComment == null ? (
                ...
                <View>
                  <View style={styles.contentContainer}>
                    <View style={styles.contentBox}>
                        ...
                        <TouchableOpacity
                          onPress={async () => {
                            if (state.currentContent.postUserId._id == userState.myInfo._id) {
                              ...
                              push('OtherAccount', {
                                otherUserId: state.currentContent.postUserId._id,
                              });
                            }
                          }}
                        >
                          {state.currentContent.postUserId.profileImage == undefined ? (
                            <View style={styles.profile}>
                              <SvgUri
                                width="100%"
                                height="100%"
                                source={require('assets/icons/noprofile.svg')}
                              />
                            </View>
                          ) : (
                            <Image
                              style={styles.profile}
                              source={{ uri: state.currentContent.postUserId.profileImage }}
                            />
                          )}
                        </TouchableOpacity>
                        <View style={styles.editContainer}>
                          <View style={{ marginLeft: 16 * tmpWidth }}>
                            <Text style={styles.nameText}>
                              {state.currentContent.postUserId.name}
                            </Text>
                          ...
                          {reportModal ? (
                            <ReportModal
                              ...
                              type="boardContent"
                              subjectId={state.currentContent._id}
                            />
                          ) : null}
                        </View>
                      </View>
                      ...
                      {(state.currentContent.song != null ||
                        state.currentContent.song != undefined) && (
                        <View style={styles.musicBox}>
                          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            <SongImage
                              url={state.currentContent.song.attributes.artwork.url}
                              size={38}
                              border={38}
                            />
                            <View style={{ marginLeft: 15 * tmpWidth, width: 160 * tmpWidth }}>
                              <Text style={{ fontSize: 13 * tmpWidth }} numberOfLines={1}>
                                {state.currentContent.song.attributes.name}
                              </Text>
                              <Text
                                style={{
                                ...
                                numberOfLines={1}
                              >
                                {state.currentContent.song.attributes.artistName}
                              </Text>
                            </View>
                          </View>
                          <TouchableOpacity
                          ...
                          >
                            {isPlayingId != state.currentContent.song.id ? (
                              <SvgUri
                                width="36"
                                height="36"
                                source={require('../../assets/icons/boardMusicPlay.svg')}
                              />
                            ) : (
                              <SvgUri
                                width="36"
                                height="36"
                                source={require('../../assets/icons/boardMusicStop.svg')}
                              />
                            )}
                            <HarmfulModal />
                          </TouchableOpacity>
                      ...
                      {state.currentContent != null &&
                      state.currentContent.image.length == 0 ? null : (
                        <FlatList
                      ...
                      >
                        <View style={{ alignItems: 'center', flexDirection: 'row' }}>
                          {state.currentContent.likes.includes(userState.myInfo._id) ? (
                            <TouchableOpacity
                              onPress={async () =>
                                await unlikeContent({ contentId: state.currentContent._id })
                              }
                            >
                              <SvgUri
                                width="24"
                                height="24"
                                source={require('assets/icons/boardLike.svg')}
                                style={{ marginRight: 2 * tmpWidth }}
                              />
                            </TouchableOpacity>
                          ) : (
                            <TouchableOpacity
                              onPress={async () =>
                                await likeContent({ contentId: state.currentContent._id })
                              }
                            >
                              <SvgUri
                                width="24"
                                height="24"
                                source={require('assets/icons/boardUnlike.svg')}
                                style={{ marginRight: 2 * tmpWidth }}
                              />
                            </TouchableOpacity>
                          )}
                          <Text style={styles.footerText}>
                            좋아요 {state.currentContent.likes.length}
                          </Text>
                        ...
                        <View style={{ alignItems: 'center', flexDirection: 'row' }}>
                          <SvgUri
                            width="24"
                            height="24"
                            source={require('assets/icons/boardComment.svg')}
                            style={{ marginRight: 2 * tmpWidth }}
                          />
                          <Text style={styles.footerText}>
                            댓글 {state.currentContent.comments.length}
                          </Text>
                        </View>
                      </View>
                    </View>
                  ...
                  <View style={{ flexDirection: 'row' }}>
                    {userState.myInfo.profileImage == undefined ? (
                      ...
                      <Image
                        ...
                        source={{ uri: userState.myInfo.profileImage }}
                      />
                    )}
      ...
      {deletedModal ? (
        <DeletedModal deletedModal={deletedModal} setDeletedModal={setDeletedModal} type="board" />
      ) : null}
    </View>
  );
};
