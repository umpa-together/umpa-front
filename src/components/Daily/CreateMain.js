...
const CreateMain = ({ isEdit, setIsSearch, daily }) => {
  const { state } = useContext(DailyContext);
  const [keyboardHeight, setKeyboardHeight] = useState(0);
  const { informationRef, setTitle, setImage, imagecheck } = useDaily();
  ...
  return (
    <View style={styles.container}>
      <Header title={`데일리${isEdit ? '수정하기' : '만들기'}`} click="업로드" isEdit={isEdit} />
