...
const CreateHashtag = () => {
  const { informationRef, image } = useDaily();
  ...
  const onSubmitEditing = () => {
    if (
      !pattern_spc.test(hashtagRef.current.value) &&
      (pattern_eng.test(hashtagRef.current.value) ||
        pattern_kor.test(hashtagRef.current.value) ||
        pattern_num.test(hashtagRef.current.value))
    ) {
  ...
  return (
    <>
      <View style={styles.container}>
        <View style={styles.flexRow}>
          ...
          <TextInput
            ...
            maxLength={9}
          />
          <TouchableOpacity style={styles.plus} onPress={onSubmitEditing}>
            ...
            <View style={[styles.flexRow, styles.margin]}>
              ...
              <TouchableOpacity style={styles.icon} onPress={() => deleteHashtag(item)}>
                <SvgUri
                  width="19"
                  height="19"
                  source={require('assets/icons/playlistDelete.svg')}
                />
              </TouchableOpacity>
            </View>
          )}
      ...
      {checkModal && (
        <Animated.View style={[styles.warningBox, { opacity }]}>
          <Animated.Text style={[styles.warningText, { opacity }]}>
            특수문자는 불가능합니다.
          </Animated.Text>
        </Animated.View>
      )}
