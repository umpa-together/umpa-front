...
import { SendFeed } from 'lib/utils/kakaoShare';
...
const DailysLikes = ({ daily }) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.flexRow}>
        <TouchableOpacity onPress={() => SendFeed({ daily })}>
        ...
        <TouchableOpacity onPress={onClickLikes}>
          <SvgUri
            ...
            height={40}
            source={
              likes.includes(userState.myInfo._id)
                ? require('assets/icons/playlisthearto.svg')
                : require('assets/icons/playlistheart.svg')
            }
          />
        </TouchableOpacity>
      </View>
      {deleteModal && (
        <DeleteModal deleteModal={deleteModal} setDeleteModal={setDeleteModal} type="daily" />
      )}
      {reportModal && (
        <ReportModal
