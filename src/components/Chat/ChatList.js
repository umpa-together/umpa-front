...
const ChatList = ({ data }) => {
  const { state, getSelectedChat, getMessagesNum, getChatList, nextChatList } =
    ...
    if (text) {
      setResult(
        data.filter(({ participate }) => {
          const target =
            participate[0]._id === userState.myInfo._id ? participate[1] : participate[0];
                  ...
                  <View style={[styles.flexRow, styles.space]}>
                    ...
                    <View style={styles.flexRow}>
                      <Text style={styles.messages}>
                        {!isLastTargetUser && messages[messages.length - 1].isRead && '읽음'}
                      </Text>
