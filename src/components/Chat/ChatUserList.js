...
const ChatUserList = ({ data }) => {
  ...
  useEffect(() => {
    if (text) {
      setResult(data.filter(({ name }) => name.includes(text)));
    } else {
      setResult(data);
    }
  }, [text]);
  ...
  return (
    <View style={styles.flex}>
      <FlatList
        ...
        renderItem={({ item }) => {
          const { profileImage, name, realName } = item;
          return (
            <TouchableOpacity style={styles.userContainer} onPress={() => onClickChat(item)}>
