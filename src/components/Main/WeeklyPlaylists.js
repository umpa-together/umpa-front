    ...
    <>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <TouchableOpacity activeOpacity={1} onPress={() => onClickPlaylist(_id, postUser._id)}>
              <Image style={styles.playlistImg} source={{ uri: image }} />
              <Text style={styles.title} numberOfLines={2}>
                {title}
              </Text>
              {hashtag.length !== 0 && (
                <Text style={styles.hashtag}>{hashtag.map((text) => ` #${text}`)}</Text>
              )}
...
export default WeeklyPlaylists;
