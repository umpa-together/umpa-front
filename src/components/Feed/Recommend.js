      ...
      <View style={styles.opt}>
        <View style={styles.optleft}>
          <TouchableOpacity style={styles.opt1} onPress={() => setResult('playlist')}>
            {result === 'playlist' ? (
              <Text style={{ fontSize: 18 * tmpWidth }}>플레이리스트</Text>
            ) : (
              <Text style={{ fontSize: 18 * tmpWidth, color: 'gray' }}>플레이리스트</Text>
            )}
          </TouchableOpacity>
          <TouchableOpacity style={styles.opt2} onPress={() => setResult('curating')}>
            {result === 'curating' ? (
              <Text style={{ fontSize: 18 * tmpWidth }}>큐레이션</Text>
            ) : (
              <Text style={{ fontSize: 18 * tmpWidth, color: 'gray' }}>큐레이션</Text>
            )}
          </TouchableOpacity>
        </View>
      </View>
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        ...
        <Animated.FlatList
          ...
          contentContainerStyle={{
            ...
            paddingLeft: 54.5 * tmpWidth,
            paddingRight: 54.5 * tmpWidth,
          }}
          decelerationRate={0}
          bounces={false}
          ...
          renderItem={({ item, index }) => {
            ...
            return (
              ...
              >
                ...
                <TouchableOpacity
                  style={styles.representSongBox}
                  onPress={() => {
                    setRepresentModal(true);
                    setRepresentSong(item.songs);
                  }}
                >
                  <Text style={styles.representText} numberOfLines={1}>
                    대표곡 - {item.songs[0].attributes.name}
                  </Text>
                </TouchableOpacity>
                <View
                  style={{
                    flexDirection: 'row',
                    marginTop: item.playlist.length === 0 ? 0 : 18 * tmpWidth,
                  }}
                >
                ...
                {isFollow.includes(index) ? (
                  <TouchableOpacity
                    ...
                    onPress={async () => {
                      ...
                      setIsFollow(isFollow.filter((followIdx) => followIdx !== index));
                    }}
                  >
