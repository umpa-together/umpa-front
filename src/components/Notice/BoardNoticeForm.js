        ...
        <Text style={styles.name} numberOfLines={2}>
          ...
          {type === 'blike' ? (
            <Text style={styles.contentText}>
              {' '}
              님이 게시글: {boardcontent.content}을 좋아합니다.{' '}
              <Text style={styles.time}>{time}</Text>
            </Text>
          ) : type === 'bcom' ? (
            <Text style={styles.contentText}>
              {' '}
              님이 게시글 {boardcontent.content}에 댓글을 달았습니다: {boardcomment.comment}{' '}
              <Text style={styles.time}>{notice.time}</Text>
            </Text>
          ) : type === 'bcomlike' ? (
            <Text style={styles.contentText}>
              {' '}
              님이 게시글 {boardcontent.content}의 댓글: {boardcomment.comment}을 좋아합니다.{' '}
              <Text style={styles.time}>{notice.time}</Text>
            </Text>
          ) : type === 'brecom' ? (
            <Text style={styles.contentText}>
              {' '}
              님이 게시글 {boardcontent.content}에 대댓글을 달았습니다: {boardrecomment.comment}{' '}
              <Text style={styles.time}>{notice.time}</Text>
            </Text>
          ) : type === 'brecomlike' ? (
            <Text style={styles.contentText}>
              {' '}
              님이 게시글 {boardcontent.content}의 대댓글: {boardrecomment.comment}을 좋아합니다.{' '}
              <Text style={styles.time}>{notice.time}</Text>
            </Text>
          ) : type === 'bsonglike' ? (
            <Text style={styles.contentText}>
              {' '}
              님이 {boardsong.song.attributes.artistName} - {boardsong.song.attributes.name}를
              좋아합니다. <Text style={styles.time}>{notice.time}</Text>
            </Text>
          ) : null}
        </Text>
      </View>
